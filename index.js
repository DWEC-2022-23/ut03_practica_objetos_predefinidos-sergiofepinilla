...
function reyesMagos(fecha) {
  ...
  if (fecha.length == 10) {
    ...
    if (date1 instanceof Date && !isNaN(date1)) {
      //Declaro variable con la fecha de los reyes magos
      let date2 = new Date("01/06/" + date1.getFullYear());
      //+1 al año si la fecha introducida es mayor
      if (date1 > date2) date2 = new Date("01/06/" + (date1.getFullYear() + 1));
      let tiempoDiferencia = date2.getTime() - date1.getTime();
      let diasDiferencia = tiempoDiferencia / (1000 * 3600 * 24);
      //Fixeo resultados
      let fixedDiasDirefencia = diasDiferencia.toFixed(0);
      if (fixedDiasDirefencia == 0) {
        document.getElementById("resultadoReyes").innerText =
          "Resultado : Hoy es Reyes Magos!";
      } else {
        document.getElementById("resultadoReyes").innerText =
...
function randomNum(comienzo, fin) {
  ...
  document.getElementById("resultadoNumAleatorio").innerText =
    "Numero Aleatorio :" +
    Math.floor(Math.random() * (Number(fin) - comienzo) + comienzo);
}
